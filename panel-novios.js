...
const EVENT_LABELS = { civil: 'Solo acto civil', cena: 'Solo cena', ambas: 'Acto civil y cena' };
// Sin dato (confirmaciones de la invitacion original) se asume asistencia completa.
function asisteACena(record) {
  return record.attend === 'yes' && record.events !== 'civil';
}
function asisteACivil(record) {
  return record.attend === 'yes' && record.events !== 'cena';
}
...
function normalizeStoredRecord(record) {
  ...
  const events = attend === 'yes' && EVENT_LABELS[eventsRaw] ? eventsRaw : '';
  const dietary = attend === 'yes' && events !== 'civil' ? (record?.dietary || '').toString().trim() : '';
  ...
  return {
    ...
    groupName,
    events,
    eventsLabel: EVENT_LABELS[events] || 'Sin especificar',
    dietary,
...
function buildGuestGroups(records) {
  ...
  records
    ...
    .forEach((record) => {
      ...
      group.eventos.push(record.eventsLabel);
      if (record.dietary) group.dietas.push(`${record.fullName}: ${record.dietary}`);
    });
  ...
  return Array.from(grouped.values())
    .map((group) => ({
      ...
      resumenEventos: Array.from(new Set(group.eventos)).join(' · '),
...
function getActiveUnits() {
  ...
  return cachedGroupsCena.map((group) => ({
    key: group.key,
...
function renderStats(records, groups) {
  ...
  const personasCivil = records.filter(asisteACivil).reduce((sum, record) => sum + record.peopleCount, 0);
  const personasCena = records.filter(asisteACena).reduce((sum, record) => sum + record.peopleCount, 0);
  const conRestricciones = records.filter((record) => record.dietary).length;
  const setStat = (id, value) => { const el = document.getElementById(id); if (el) el.textContent = String(value); };
  setStat('stat-civil', personasCivil);
  setStat('stat-cena', personasCena);
  setStat('stat-dietary', conRestricciones);
}
...
function renderGroupedList(groups, query = '') {
  ...
  visibleGroups.forEach((group) => {
    ...
    card.appendChild(createElement('p', 'group-meta', `Asiste a: ${group.resumenEventos}`));
    if (group.dietas.length) card.appendChild(createElement('p', 'group-diet', `Restricciones: ${group.dietas.join(' · ')}`));
...
function renderDietaryList(records) {
  if (!dietaryList) return;
  clearChildren(dietaryList);

  const filas = records
    .filter((record) => record.dietary)
    .sort((a, b) => a.fullName.localeCompare(b.fullName, 'es', { sensitivity: 'base' }));

  if (!filas.length) {
    dietaryList.appendChild(createElement('p', 'empty-note', 'Nadie ha indicado restricciones alimenticias.'));
    return;
  }

  filas.forEach((record) => {
    const item = createElement('article', 'extras-item');
    item.appendChild(createElement('p', 'extras-item__main', record.dietary));
    item.appendChild(createElement('p', 'extras-item__by', `${record.fullName} · ${record.eventsLabel} · ${record.peopleCount} ${record.peopleCount === 1 ? 'persona' : 'personas'}`));
...
async function refreshDashboard(statusMessage = '', statusType = 'success') {
  ...
  // Las mesas son de la cena: quien solo va al acto civil no ocupa silla.
  cachedGroupsCena = buildGuestGroups(records.filter(asisteACena));
  cachedIndividuals = buildGuestIndividuals(records.filter(asisteACena));
...
function updateTabCounts(records, groups) {
  ...
  setCount(
    'tab-count-detalles',
    records.filter((record) => record.song || record.message || record.dietary).length
  );
}
